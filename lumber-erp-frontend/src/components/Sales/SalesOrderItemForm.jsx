import React, { useState, useEffect } from 'react';
import { X, Save, List } from 'lucide-react';
import { salesOrderItemsAPI, salesOrdersAPI, productTypesAPI } from '../../services/api';

const SalesOrderItemForm = ({ item, onClose, onSuccess }) => {
  const [formData, setFormData] = useState({
    so_id: '',
    product_type_id: '',
    quantity_sold: '',
    unit_price: '',
    discount: '',
  });
  const [salesOrders, setSalesOrders] = useState([]);
  const [productTypes, setProductTypes] = useState([]);
  const [loading, setLoading] = useState(false);
  const [errors, setErrors] = useState({});

  useEffect(() => {
    fetchData();
    if (item) {
      setFormData({
        so_id: item.soid || '', // Backend returns 'soid'
        product_type_id: item.product_type_id || '',
        quantity_sold: item.quantity || '', // Backend returns 'quantity'
        unit_price: item.unit_price || '',
        discount: item.discount || '0',
      });
    }
  }, [item]);

  const fetchData = async () => {
    try {
      const [soRes, ptRes] = await Promise.all([
        salesOrdersAPI.getAll(),
        productTypesAPI.getAll(),
      ]);
      setSalesOrders(soRes.data);
      setProductTypes(ptRes.data);
    } catch (error) {
      console.error('Error fetching data:', error);
    }
  };

  const validateForm = () => {
    const newErrors = {};
    if (!formData.so_id) newErrors.so_id = 'Sales order is required';
    if (!formData.product_type_id) newErrors.product_type_id = 'Product type is required';
    if (!formData.quantity_sold) newErrors.quantity_sold = 'Quantity is required';
    if (!formData.unit_price) newErrors.unit_price = 'Unit price is required';
    
    setErrors(newErrors);
    return Object.keys(newErrors).length === 0;
  };

  const handleChange = (e) => {
    const { name, value } = e.target;
    setFormData(prev => ({ ...prev, [name]: value }));
    if (errors[name]) {
      setErrors(prev => ({ ...prev, [name]: '' }));
    }
  };

  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!validateForm()) return;

    setLoading(true);
    try {
      const quantity = parseFloat(formData.quantity_sold);
      const unitPrice = parseFloat(formData.unit_price);
      const discount = parseFloat(formData.discount) || 0;
      const subtotal = (quantity * unitPrice) - discount;

      // Map frontend fields to backend expected fields
      const submitData = {
        soid: parseInt(formData.so_id), // Backend expects 'soid'
        product_type_id: parseInt(formData.product_type_id),
        quantity: quantity, // Backend expects 'quantity'
        unit_price: unitPrice,
        discount: discount, // Backend expects 'discount'
        subtotal: subtotal, // Backend expects 'subtotal'
      };

      if (item) {
        await salesOrderItemsAPI.update(item.so_item_id, submitData);
      } else {
        await salesOrderItemsAPI.create(submitData);
      }
      onSuccess();
    } catch (error) {
      console.error('Error saving SO item:', error);
      alert('Error saving SO item. Please try again.');
    } finally {
      setLoading(false);
    }
  };

  const calculateTotal = () => {
    const qty = parseFloat(formData.quantity_sold) || 0;
    const price = parseFloat(formData.unit_price) || 0;
    const disc = parseFloat(formData.discount) || 0;
    return (qty * price - disc).toFixed(2);
  };

  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        <div className="modal-header">
          <div className="modal-title-wrapper">
            <List size={24} />
            <h2>{item ? 'Edit SO Item' : 'Add SO Item'}</h2>
          </div>
          <button className="btn btn-icon" onClick={onClose}>
            <X size={20} />
          </button>
        </div>

        <form onSubmit={handleSubmit}>
          <div className="modal-body">
            <div className="input-group">
              <label htmlFor="so_id">Sales Order *</label>
              <select
                id="so_id"
                name="so_id"
                value={formData.so_id}
                onChange={handleChange}
                className={errors.so_id ? 'error' : ''}
              >
                <option value="">Select SO</option>
                {salesOrders.map(so => (
                  <option key={so.soid} value={so.soid}>
                    SO #{so.soid} - {so.status}
                  </option>
                ))}
              </select>
              {errors.so_id && <span className="error-text">{errors.so_id}</span>}
            </div>

            <div className="input-group">
              <label htmlFor="product_type_id">Product Type *</label>
              <select
                id="product_type_id"
                name="product_type_id"
                value={formData.product_type_id}
                onChange={handleChange}
                className={errors.product_type_id ? 'error' : ''}
              >
                <option value="">Select Product</option>
                {productTypes.map(pt => (
                  <option key={pt.product_type_id} value={pt.product_type_id}>
                    {pt.product_name} - ${pt.unit_price}
                  </option>
                ))}
              </select>
              {errors.product_type_id && <span className="error-text">{errors.product_type_id}</span>}
            </div>

            <div className="form-grid">
              <div className="input-group">
                <label htmlFor="quantity_sold">Quantity *</label>
                <input
                  id="quantity_sold"
                  name="quantity_sold"
                  type="number"
                  step="0.01"
                  value={formData.quantity_sold}
                  onChange={handleChange}
                  className={errors.quantity_sold ? 'error' : ''}
                />
                {errors.quantity_sold && <span className="error-text">{errors.quantity_sold}</span>}
              </div>

              <div className="input-group">
                <label htmlFor="unit_price">Unit Price ($) *</label>
                <input
                  id="unit_price"
                  name="unit_price"
                  type="number"
                  step="0.01"
                  value={formData.unit_price}
                  onChange={handleChange}
                  className={errors.unit_price ? 'error' : ''}
                />
                {errors.unit_price && <span className="error-text">{errors.unit_price}</span>}
              </div>
            </div>

            <div className="input-group">
              <label htmlFor="discount">Discount ($)</label>
              <input
                id="discount"
                name="discount"
                type="number"
                step="0.01"
                value={formData.discount}
                onChange={handleChange}
                placeholder="0.00"
              />
            </div>

            {formData.quantity_sold && formData.unit_price && (
              <div className="total-preview">
                <strong>Subtotal:</strong> ${calculateTotal()}
              </div>
            )}
          </div>

          <div className="modal-footer">
            <button type="button" className="btn btn-secondary" onClick={onClose}>
              Cancel
            </button>
            <button type="submit" className="btn btn-primary" disabled={loading}>
              <Save size={18} />
              {loading ? 'Saving...' : item ? 'Update' : 'Create'}
            </button>
          </div>
        </form>
      </div>
    </div>
  );
};

export default SalesOrderItemForm;